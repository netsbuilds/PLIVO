"""Trainer with warmup+cosine LR schedule, grad clipping, and tuned AdamW.
...
def get_lr(step, total_steps, peak_lr, warmup_frac=0.05, min_lr_frac=0.1):
...
def main():
    ...
    ap.add_argument("--lr", type=float, default=6e-4)   # <- peak LR, was 3e-4
    ...
    opt = torch.optim.AdamW(
        model.parameters(), lr=args.lr, betas=(0.95, 0.999), weight_decay=0.1
    )
    ...
    for step in range(1, args.steps + 1):
        lr = get_lr(step - 1, args.steps, args.lr)
